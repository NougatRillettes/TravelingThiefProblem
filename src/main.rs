...
fn new_gen<T: Rand>() -> mpsc::Receiver<T>
    ...
    for loop_n in 0..10 {
        ...
        while (i - last_it) <= (max_it as i64) {
            ...
            if tour.rls_try_one(&recv_rls, temp).0 {
                println!("[ RLS ] New cost : {:?} in {} iterations ({}/{} since last output) \
                          (temp : {:e})",
                         tour.cost,
                         i,
                         i - last_it,
            ...
            if i % 4 == 0 {
                if tour.two_opt_rand(&recv_two_opt) {
                    println!("[2-opt] New cost : {:?} in {} iterations ({} since last output) \
                              (temp : {:e})",
                             tour.cost,
                             i,
                             i - last_it,
            ...
            if i % 4 == 0 {
                if tour.stich_try_one(&recv_stich, temp).0 {
                    println!("[stich] New cost : {:?} in {} iterations ({} since last output) \
                              (temp : {:e})",
                             tour.cost,
                             i,
                             i - last_it,
            ...
            if i % 1024 == 0 && (time::now() - start_time) >= time::Duration::minutes(3) {
                break;
            }
